...
function validarDatosUsuario(usuario: Usuario, validarContrasena: boolean): Usuario {
...
export async function agregarUsuario(usuario: Usuario): Promise<Usuario> {
  ...
  const contrasenaCifrada = await bcrypt.hash(nuevoUsuario.contrasena as string, 12);
...
export async function actualizarUsuario(id: number, datos: Usuario): Promise<Usuario | null> {
  if (!validarId(id)) {
    throw new Error('ID inválido.');
  }
  ...
  if (usuarioActualizado.contrasena) {
    const contrasenaCifrada = await bcrypt.hash(usuarioActualizado.contrasena, 12);
