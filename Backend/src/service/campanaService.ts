...
function fecha(data: Record<string, unknown>, key: string): string {
    ...
    if (!/^\d{4}-\d{2}-\d{2}$/.test(valor) || valor.slice(0, 4) === "0000" || !Number.isFinite(parseada.getTime()) || parseada.toISOString().slice(0, 10) !== valor) {
...
export function validarCampana(entrada: unknown) {
    ...
    if (fechaFin < fechaInicio) throw new ApiError(400, "La fecha final no puede ser anterior a la inicial.");
    const estado = texto(data, "estado", 20);
    if (!estadosCampana.includes(estado as EstadoCampana)) throw new ApiError(400, "Estado de campaña inválido.");
    const imagenUrl = texto(data, "imagenUrl", 300, false);
    if (imagenUrl) {
        try {
            const url = new URL(imagenUrl);
            if (!["https:", "http:"].includes(url.protocol) || url.username || url.password) throw new Error();
        } catch { throw new ApiError(400, "La imagen debe ser una URL HTTP o HTTPS válida."); }
    }
    return {
        ...
        fechaInicio, fechaFin, estado: estado as EstadoCampana, imagenUrl: imagenUrl || null
    };
}
...
export function leerFiltrosCampana(query: Record<string, unknown>) {
    ...
    if (estado && !estadosCampana.includes(estado as EstadoCampana)) throw new ApiError(400, "Estado de campaña inválido.");
...
export async function listarCampanas(actor: Actor, filtros: ReturnType<typeof leerFiltrosCampana>) {
    ...
    const [listado, cantidad] = await Promise.all([
        pool.query<Campana>(
            ...
            + condicion + " ORDER BY c.fecha_inicio DESC,c.id_campana DESC LIMIT $4 OFFSET $5",
            [...params,porPagina,(filtros.pagina-1)*porPagina]),
        pool.query<{total: number}>("SELECT count(*)::integer AS total"+condicion,params)
    ]);
    return { campanas: listado.rows, total: cantidad.rows[0].total, pagina: filtros.pagina, porPagina, puedeGestionar: puedeGestionar(actor) };
}

export async function guardarCampana(entrada: unknown, actor: Actor, id?: number) {
    if (!puedeGestionar(actor)) throw new ApiError(403, "Solo administradores y supervisores pueden gestionar campañas.");
    ...
    return transaccion(async db => {
        ...
        if (datos.ubicacion) {
            ...
            // No se modifica una ubicación que pueda estar vinculada a otros registros.
            const existente = idUbicacionAnterior === null ? null : await db.query(
                ...
                [idUbicacionAnterior,lugar.departamento,lugar.municipio,lugar.zona || null,lugar.direccion || null,lugar.referencia || null]
            );
            idUbicacion = existente?.rows.length
                ? idUbicacionAnterior
                : (await agregarUbicacion(lugar,db)).idUbicacion!;
        }
        const valores = [idUbicacion,datos.titulo,datos.descripcion,datos.organizador,datos.fechaInicio,datos.fechaFin,datos.imagenUrl,datos.estado];
