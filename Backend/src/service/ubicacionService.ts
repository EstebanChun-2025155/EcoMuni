...
export async function actualizarUbicacion(id: number, datos: Ubicacion): Promise<Ubicacion | null> {
  if (!validarId(id)) {
    throw new Error('ID inválido.');
  }
...
export async function eliminarUbicacion(id: number): Promise<Ubicacion | null> {
  if (!validarId(id)) {
    throw new Error('ID inválido.');
  }
