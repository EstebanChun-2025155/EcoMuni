...
function validarDatosCategoria(categoria: Categoria): Categoria {
  objeto(categoria);
  const errores = validarCategoria(categoria.categoria, categoria.descripcion, categoria.estado);
...
export async function actualizarCategoria(id: number, datos: Categoria): Promise<Categoria | null> {
  if (!validarId(id)) {
    throw new Error('ID inválido.');
  }
...
export async function listarCategoriasActivas(): Promise<Categoria[]> {
  return (
    await pool.query<Categoria>(
      `SELECT id_categoria AS "idCategoria", categoria, descripcion, estado FROM categoria WHERE estado='activa' ORDER BY id_categoria`,
    )
  ).rows;
}
