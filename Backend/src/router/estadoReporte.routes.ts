...
import { agregarEstado, actualizarEstado, buscarEstado, eliminarEstado, listarEstados } from "../service/estadoReporteService";
...
estadoReporteRouter.put("/actualizarEstado/:id", exigirGestion, async (req: Request, res: Response) => {
    try {
...
estadoReporteRouter.delete("/eliminarEstado/:id", exigirGestion, async (req: Request, res: Response) => {
    try {
        ...
        res.status(200).json({ mensaje: `Estado con ID ${estado.idEstado} eliminado correctamente.` });
    } catch (error) {
        responderError(res, error);
    }
});
