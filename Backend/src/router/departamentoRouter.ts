...
router.param('slug', (req, res, next, value) => {
  try {
    res.locals.departamento = nombreDepartamento(value);
    next();
  } catch (error) {
    next(error);
  }
...
router.get('/:slug/reportes/:id', async (req, res) => {
  res.json(
    await detalleReporte(res.locals.departamento, idParametro(req.params.id), res.locals.actor),
  );
});
...
router.post('/:slug/reportes/:id/comentarios', async (req, res) => {
  res
    ...
    .json(
      await comentar(
        res.locals.departamento,
        idParametro(req.params.id),
        req.body,
        res.locals.actor,
      ),
    );
...
router.post('/:slug/reportes/:id/apoyo', async (req, res) => {
  res.json(
    await apoyar(res.locals.departamento, idParametro(req.params.id), res.locals.actor, true),
  );
});
router.delete('/:slug/reportes/:id/apoyo', async (req, res) => {
  res.json(
    await apoyar(res.locals.departamento, idParametro(req.params.id), res.locals.actor, false),
  );
});
router.post('/:slug/reportes/:id/seguimientos', exigirGestion, async (req, res) => {
  res
    .status(201)
    .json(
      await seguir(res.locals.departamento, idParametro(req.params.id), req.body, res.locals.actor),
    );
});
...
router.patch('/:slug/reportes/:id/evidencias/:idEvidencia', async (req, res) => {
  res.json(
    await describirEvidencia(
      res.locals.departamento,
      idParametro(req.params.id),
      idParametro(req.params.idEvidencia),
      req.body,
      res.locals.actor,
    ),
  );
