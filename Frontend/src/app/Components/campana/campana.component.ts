...
export class Campana {
  ...
  get fechasValidas(): boolean {
    return (
      ...
      this.formulario.fechaFin >= this.formulario.fechaInicio
    );
  }
  recargar(): void {
    if (!this.guardando()) this.recargas.next();
  }
  buscar(): void {
  ...
  abrirFormulario(campana?: CampanaRegistro): void {
    ...
    this.formulario = campana
      ? {
          ...
          ubicacion: campana.ubicacion ? { ...campana.ubicacion } : null,
        }
      : campanaVacia();
  ...
  guardar(): void {
    ...
    const datos: DatosCampana = {
      ...this.formulario,
      ubicacion: this.conUbicacion ? { ...this.lugar } : null,
    };
    this.api
      .guardar(datos, this.editando)
      .pipe(
        takeUntilDestroyed(this.destroyRef),
  ...
  private informarError(error: unknown): void {
    if (error instanceof HttpErrorResponse) {
      ...
      if (typeof error.error?.mensaje === 'string') {
        this.error.set(error.error.mensaje);
        return;
      }
    }
